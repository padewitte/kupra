...
public class KupraRestRouteBuilder extends RouteBuilder {
    ...
    public void configure() {
        ...
        rest("/{CamelMongoDbDatabase}").description("Database operations").consumes("exchange").produces("application/json")
                ...
                .get("/{CamelMongoDbCollection}/count").description("Cunt")
                    .route().process(new KupraProcessor()).to(String.format(mongoDBRoute, MongoDbOperation.count)).process(new KupraOutProcessor()).endRest()
                .post("/{CamelMongoDbCollection}").description("Content of body is insert in collection.")
                    .route().process(new KupraProcessor()).to(String.format(mongoDBRoute, MongoDbOperation.insert)).process(new KupraOutProcessor()).onException(CamelMongoDbException.class).handled(true).process(new KupraErrorProcessor()).endRest()
